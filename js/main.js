...
let elAz = document.querySelector('.js-a-z');

elAz.addEventListener('click', () => {
	elList.innerHTML = '';

	let newAz = films.sort((a, b) => {
		if (a.title > b.title) return 1;
		return -1;
	});

	appendToDom(newAz, elList);
});



let elZa = document.querySelector('.js-z-a');

elZa.addEventListener('click', () => {
	elList.innerHTML = '';

	let newZa = films.sort((a, b) => {
		if (a.title < b.title) return 1;
		return -1;
	});

	appendToDom(newZa, elList);
});
